...
for idx, place in enumerate(places, start=1):
    ...
    ground_path = os.path.join(base_path, "ground")
    aerial_path = os.path.join(base_path, "aerial")
    os.makedirs(ground_path, exist_ok=True)
    os.makedirs(aerial_path, exist_ok=True)

    print(f"\n📍 {idx}. {place}")

    # Скачивание фото с земли
    urls_ground = get_image_urls(place + " фото с земли", IMAGES_PER_PLACE * 2)
    count = 0
    for url in tqdm(urls_ground, desc=f"С земли {place}", total=IMAGES_PER_PLACE):
        try:
            img_data = requests.get(url, headers=HEADERS, timeout=10).content
            with open(os.path.join(ground_path, f"photo_{count + 1}.jpg"), "wb") as f:
                f.write(img_data)
            count += 1
    ...
    # Скачивание фото с воздуха
    urls_aerial = get_image_urls(place + " фото с воздуха", IMAGES_PER_PLACE * 2)
    count = 0
    for url in tqdm(urls_aerial, desc=f"С воздуха {place}", total=IMAGES_PER_PLACE):
        try:
            img_data = requests.get(url, headers=HEADERS, timeout=10).content
            with open(os.path.join(aerial_path, f"photo_{count + 1}.jpg"), "wb") as f:
                f.write(img_data)
            count += 1
            if count >= IMAGES_PER_PLACE:
                break
